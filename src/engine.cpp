...
int fromCDT(Lldbmi2* pstate, const char* commandLine, int linesize) // from cdt
{
    ...
    // MISCELLANOUS COMMANDS
        ...
        pstate->launchInfo.SetWorkingDirectory(path);
        logprintf(LOG_NONE, "cwd=%s pwd=%s\n", path, pstate->launchInfo.GetWorkingDirectory());
        cdtprintf("%d^done\n(gdb)\n", cc.sequence);
    ...
    // TARGET AND RUN COMMANDS
        ...
        if (pstate->target.GetProcess().IsValid()) {
            if (pstate->target.GetProcess().GetState() ==
    ...
    // OTHER COMMANDS
        ...
        SBBreakpoint breakpoint =
            pstate->target.BreakpointCreateForException(lldb::LanguageType::eLanguageTypeC_plus_plus, true, false);
        ...
        SBBreakpoint breakpoint =
            pstate->target.BreakpointCreateForException(lldb::LanguageType::eLanguageTypeC_plus_plus, false, true);
                ...
                if (ilist.IsValid()) {
                    ...
                    for (int i = 0; i < cnt; i++) {
                        ...
                        cdtprintf("{address=\"%p\",func-name=\"%s\",offset=\"%d\",inst=\"%-12s%-25s %s\"}",
                                  iaddr.GetFileAddress(), symb.GetName(), off, instr.GetMnemonic(pstate->target),
                                  instr.GetOperands(pstate->target), instr.GetComment(pstate->target));
                    }
                    cdtprintf("]\n(gdb)\n");
                } else

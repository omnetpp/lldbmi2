...
bool getPeudoArrayVariable(SBFrame frame, const char* expression, SBValue& var) {
    ...
    *pe = '\0';
    ...
    snprintf(newexpression, sizeof(newexpression),
             "%s*(%s[%s])&%s[%s]%s", // (char(*)[100])&c[0] or &((char(*)[100])&c[0])
...
char* strfind(char* string, const char* find, int way, const char* except) {
    ...
    if (findlen > stringlen)
        return NULL;
    for (cp = way > 0 ? string : string + stringlen - findlen; cp >= string && cp <= string + stringlen - findlen;
         cp += way) {
...
char* castexpression(SBFrame frame, const char* expression, char* newexpression, size_t expressionsize) {
    ...
    *pe++ = '\0';
    ...
    logprintf(LOG_DEBUG, "castexpression: parent=%s parenttypename=%s, parenttypedisplayname=%s\n", getName(parent),
              parenttypename, parenttype.GetDisplayTypeName());
    snprintf(newexpression, expressionsize, "(((%s)%s)->%s", parenttypename, subexpression + 2,
             pe); // a ) is left at the end
    return newexpression;
}

// get a variable by trying many approaches
SBValue getVariable(SBFrame frame, const char* expression, bool tryDirect) {
...
int updateVarState(SBValue var, int depth) {
    ...
    logprintf(LOG_DEBUG,
              ...
              "typeclass=%-10s, basictype=%-10s, bytesize=%-2d\n",
              getName(var), var.GetNumChildren(), getNameForTypeClass(vartype.GetTypeClass()),
              getNameForBasicType(vartype.GetBasicType()), vartype.GetByteSize(),
              getNameForTypeClass(vartype.GetPointeeType().GetTypeClass()),
              getNameForBasicType(vartype.GetPointeeType().GetBasicType()), vartype.GetPointeeType().GetByteSize());
    logprintf(LOG_NONE, "updateVarState: Is(%-5s) = %d %d %d %d %s\n", getName(var), var.IsValid(), var.IsInScope(),
              var.IsDynamic(), var.IsSynthetic(), var.GetError().GetCString());
    ...
    logprintf(LOG_DEBUG,
              "updateVarState: name=%s, varexpressionpath=%s, varnumchildren=%d, value=%s, summary=%s, changed=%d\n",
...
char* formatChildrenList(StringB& childrendescB, SBValue var, char* expression, int threadindexid,
                         int& varnumchildren) {
    ...
    for (ichild = 0; ichild < min(varnumchildren, limits.children_max); ichild++) {
        ...
        if (strcmp(childname, displaytypename) == 0)  // if extends class name
            ...
            if (strcmp((const char*)expression, (const char*)expressionpathdescB.c_str()) == 0) {
                SBType vartype = var.GetType();
                logprintf(LOG_ALL,
                          "formatChildrenList: Var=%-5s: children=%-2d, typeclass=%-10s, basictype=%-10s, "
                          "bytesize=%-2d, Pointee: typeclass=%-10s, basictype=%-10s, bytesize=%-2d\n",
                          getName(var), var.GetNumChildren(), getNameForTypeClass(vartype.GetTypeClass()),
                          getNameForBasicType(vartype.GetBasicType()), vartype.GetByteSize(),
                          getNameForTypeClass(vartype.GetPointeeType().GetTypeClass()),
                          getNameForBasicType(vartype.GetPointeeType().GetBasicType()),
                          vartype.GetPointeeType().GetByteSize());
    ...
    for (size_t i = 0; i < varslist.GetSize(); i++) {
        ...
        if (var.IsValid() && var.GetError().Success()) {
            ...
            logprintf(LOG_DEBUG, "formatVariables: var=%s, type class=%s, basic type=%s \n", getName(var),
                      getNameForTypeClass(vartype.GetTypeClass()), getNameForBasicType(vartype.GetBasicType()));
...
char* formatSummary(StringB& summarydescB, SBValue var) {
    ...
    SBType vartype = var.GetType();
    logprintf(LOG_DEBUG,
              "formatSummary: Var=%-5s: children=%-2d, typeclass=%-10s, basictype=%-10s, bytesize=%-2d, Pointee: "
              "typeclass=%-10s, basictype=%-10s, bytesize=%-2d\n",
              getName(var), var.GetNumChildren(), getNameForTypeClass(vartype.GetTypeClass()),
              getNameForBasicType(vartype.GetBasicType()), vartype.GetByteSize(),
              getNameForTypeClass(vartype.GetPointeeType().GetTypeClass()),
...
char* formatValue(StringB& vardescB, SBValue var, VariableDetails details) {
    ...
    logprintf(LOG_DEBUG, "formatValue: var=%p, name=%s, summary=%s, value=%s, address=%p\n", &var, varname,
              summarydescB.c_str(), varvalue, varaddr);
